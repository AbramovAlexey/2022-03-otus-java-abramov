...
class ComplexProcessorTest {
    ...
    void timeExceptionTest() {
        var message = new Message.Builder(1L).field9("field9").build();
        var processor = new ProcessorThrowEvenSecond(() -> true);
        var complexProcessor = new ComplexProcessor(List.of(processor), (ex) -> {throw new TestException(ex.getMessage());});
        assertThatExceptionOfType(TestException.class).isThrownBy(() -> complexProcessor.handle(message));

        processor.setTimeAnalyzerProvider(() -> false);
        var result = complexProcessor.handle(message);
        assertThat(result).isEqualTo(message);
    }
